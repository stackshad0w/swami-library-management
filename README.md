...
The application uses Supabase Postgres database. Below is the relational database schema structure and setup instructions.

### SQL Editor Execution

To set up the database, copy and run the SQL migration query located in [`supabase/migrations/001_initial_schema.sql`](file:///d:/tmp/swami-library-management/supabase/migrations/001_initial_schema.sql) in your Supabase Dashboard **SQL Editor**.

### Table Breakdown

#### 1. `courses`
Stores the available library study courses.
...
#### 2. `fee_tiers`
Defines fee pricing tiers based on gender, shift, and subscription duration.
- `id` (`UUID`, Primary Key, DEFAULT `uuid_generate_v4()`)
- `gender` (`TEXT`, CHECK `Male | Female | Other | All`)
- `shift` (`TEXT`, CHECK `Day | Night | Both`)
- `months` (`INT`, CHECK `1 | 2 | 3 | 6 | 12`)
- `fee` (`NUMERIC(10,2)`, DEFAULT `0`)
- *Unique Constraint*: `(gender, shift, months)`

#### 3. `students`
Main record table for library members/students.
- `id` (`UUID`, Primary Key, DEFAULT `uuid_generate_v4()`)
- `student_id` (`TEXT`, UNIQUE, e.g. `STU-0001`)
- `name` (`TEXT`, NOT NULL)
- `phone` (`TEXT`, NOT NULL)
- `email` (`TEXT`, Optional)
...
- `conditions` (`TEXT`, Optional medical/special notes)
...
- `fee_status` (`TEXT`, DEFAULT `'Active'`, CHECK `Active | Overdue | Inactive | Expired`)
- `is_active` (`BOOLEAN`, DEFAULT `TRUE`)
- `created_at` (`TIMESTAMPTZ`, DEFAULT `NOW()`)
- `updated_at` (`TIMESTAMPTZ`, DEFAULT `NOW()`, auto-updated via trigger `students_updated_at`)

#### 4. `payment_history`
Tracks payment receipts for student fee subscriptions.
- `id` (`UUID`, Primary Key, DEFAULT `uuid_generate_v4()`)
...
- `shift` (`TEXT`, CHECK `Day | Night | Both`)
- `method` (`TEXT`, CHECK `Cash | UPI | Online Transfer | Cheque | Demand Draft | Card | Free`)
- `notes` (`TEXT`, Optional)
- `created_at` (`TIMESTAMPTZ`, DEFAULT `NOW()`)

#### 5. `seat_bookings`
Manages seat assignments (Seats 1 through 84) and slot allocations.
- `id` (`UUID`, Primary Key, DEFAULT `uuid_generate_v4()`)
- `seat_number` (`INT`, CHECK `1` to `84`)
- `student_id` (`UUID`, Foreign Key -> `students(id)` ON DELETE CASCADE)
- `booking_date` (`DATE`, NOT NULL)
- `slots` (`JSONB`, Array of `{label, start, end}`)
- `fee` (`NUMERIC(10,2)`, DEFAULT `0`)
- `payment_method` (`TEXT`, CHECK `Cash | UPI | Online Transfer | Cheque | Demand Draft | Card | Free`)
- `status` (`TEXT`, DEFAULT `'Active'`, CHECK `Active | Expired | Cancelled`)
- `created_at` (`TIMESTAMPTZ`, DEFAULT `NOW()`)
- *Indexes*: `booking_date`, `student_id`, `(seat_number, booking_date)`

#### 6. `cash_closings`
Daily cash register balancing and audit logs.
- `id` (`UUID`, Primary Key, DEFAULT `uuid_generate_v4()`)
- `closing_date` (`DATE`, UNIQUE, DEFAULT `CURRENT_DATE`)
- `opening_cash` (`NUMERIC(10,2)`, DEFAULT `0`)
- `cash_collected` (`NUMERIC(10,2)`, DEFAULT `0`)
- `upi_online_total` (`NUMERIC(10,2)`, DEFAULT `0`)
- `actual_closing_cash` (`NUMERIC(10,2)`, DEFAULT `0`)
- `difference` (`NUMERIC(10,2)`, DEFAULT `0`)
- `closed_by` (`TEXT`, NOT NULL)
- `notes` (`TEXT`, Optional)
- `status` (`TEXT`, DEFAULT `'Closed'`, CHECK `Open | Closed`)
- `created_at` (`TIMESTAMPTZ`, DEFAULT `NOW()`)

#### 7. `app_settings`
Singleton configuration settings row for system defaults, themes, owner PIN, and notification templates.
- `id` (`UUID`, Primary Key, DEFAULT `uuid_generate_v4()`)
- `reminder_days_before` (`INT`, DEFAULT `5`)
- `overdue_days_threshold` (`INT`, DEFAULT `7`)
- `owner_pin` (`TEXT`, DEFAULT `'1234'`)
- `theme` (`TEXT`, DEFAULT `'dark'`)
- `accent_color` (`TEXT`, DEFAULT `'purple'`)
- `font_size` (`TEXT`, DEFAULT `'medium'`)
- `institution_name` (`TEXT`, DEFAULT `'Swami Abhyasika'`)
- `institution_admin_name` (`TEXT`, DEFAULT `'Admin'`)
- `institution_phone` (`TEXT`, DEFAULT `''`)
- `institution_email` (`TEXT`, DEFAULT `''`)
- `institution_address` (`TEXT`, DEFAULT `''`)
- `whatsapp_template_overdue` (`TEXT`)
- `whatsapp_template_upcoming` (`TEXT`)

#### 8. `pending_deletes`
Approval queue for administrative student deletion requests.
- `id` (`UUID`, Primary Key, DEFAULT `uuid_generate_v4()`)
- `student_id` (`UUID`, Foreign Key -> `students(id)` ON DELETE CASCADE)
...
CREATE POLICY "authenticated_all" ON courses         TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "authenticated_all" ON fee_tiers       TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "authenticated_all" ON students        TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "authenticated_all" ON payment_history TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "authenticated_all" ON seat_bookings   TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "authenticated_all" ON cash_closings   TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "authenticated_all" ON app_settings    TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "authenticated_all" ON pending_deletes TO authenticated USING (true) WITH CHECK (true);

-- Storage bucket setup for student photos (run in Supabase dashboard)
